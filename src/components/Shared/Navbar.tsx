        ...
        <nav className="w-full flex justify-between gap-6 relative">
          ...
          <div className="min-w-max inline-flex relative">
            <a href="/" className="relative flex items-center gap-3">
              ...
              <div className="inline-flex text-lg font-semibold text-gray-900">Enroque Bienes Raíces</div>
            </a>
          </div>

          {isOpen && (
            <div className="fixed inset-0 lg:hidden bg-gray-800/60 backdrop-filter backdrop-blur-xl" onClick={() => setIsOpen(false)}></div>
          )}

          <div
            className={`flex ${isOpen ? 'visible opacity-100 translate-y-0' : 'invisible opacity-0 translate-y-10'} lg:visible lg:opacity-100 lg:translate-y-0 lg:scale-y-100 duration-300 ease-linear flex-col gap-y-6 gap-x-4 lg:flex-row w-full lg:justify-between lg:items-center absolute lg:relative top-full lg:top-0 bg-white lg:bg-transparent border-x border-x-gray-100 lg:border-x-0`}
          >
            ...
            <div className="lg:min-w-max flex items-center sm:w-max w-full pb-6 lg:pb-0 border-b border-gray-100 lg:border-0 px-6 lg:px-0">
              <a href="#" className="flex justify-center items-center w-full sm:w-max px-6 h-12 rounded-full bg-blue-600 border-transparent hover:border-[#172554] relative overflow-hidden duration-300 ease-linear">
          ...
          <div className="min-w-max flex items-center gap-x-3">
            <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden outline-none w-7 h-auto flex flex-col relative">
